<!DOCTYPE html>
 ...
 <body>
    <div class="layui-container" style="padding:15px">
     ...
     <script>
        //  获取父页面传过来的值
        var device_event_info =  window.parent.edit_device_event_info;
        console.log(device_event_info);
        $('input[name="name"]').val(device_event_info.name);
        $('textarea[name="desc"]').val(device_event_info.desc);
        $('select[name="type"]').val(device_event_info.type);
        $('input[name="value"]').val(device_event_info.value);
        $('select[name="operation_type"]').val(device_event_info.operation_type);
        form.render('select');
         //缓存区域与房间
            ...
            success: function(result){
                if (result.code > 0) {
                    ...
                    $.each(result.data,function(key,value){
                        if(value.id == device_event_info.device_region_id){
                    ...
                    $.each(device_region_list,function(key,value){
                        if(value.id == device_event_info.device_region_id){
                            ...
                            $.each(device_region_list[key].device_room,function(k,v){
                                if(v.id == device_event_info.device_room_id){
                    ...
                    $.ajax({ 
                        type: "GET",
                        url: '{{url("api/admin/device/all")}}?device_room_id='+device_event_info.device_room_id,
                        success: function(result){
                            if (result.code > 0) {
                                layer.msg(result.msg);
                            } else {
                                ...
                                $.each(result.data,function(key,value){
                                    if(value.id == device_event_info.device_id){
                                        html+="<option value='"+value.id+"' selected>"+value.name+"</option>";
                                    }else{
                                        html+="<option value='"+value.id+"'>"+value.name+"</option>";
                                    }
                                ...
                                $.each(device_list,function(key,value){
                                    if(value.id == device_event_info.device_id){
                                        ...
                                        $.each(device_list[key].device_field,function(k,v){
                                            if(v.id == device_event_info.device_field_id){
                                                html+="<option value='"+v.id+"' selected>"+v.name+"</option>";
                                            }else{
                                                html+="<option value='"+v.id+"'>"+v.name+"</option>";
                                            }
                                ...
                                $.each(result.data,function(key,value){
                                    if(value.id != device_event_info.device_id){
                                        if(value.id == device_event_info.associated_device_id){
                                            html+="<option value='"+value.id+"' selected>"+value.name+"</option>";
                                        }else{
                                            html+="<option value='"+value.id+"'>"+value.name+"</option>";
                                        }
                                ...
                                $.each(device_list,function(key,value){
                                    if(value.id == device_event_info.associated_device_id){
                                        var html='<option value="">请选择字段</option>';
                                        $.each(device_list[key].device_field,function(k,v){
                                            if(v.id == device_event_info.associated_device_field_id){
                                                html+="<option value='"+v.id+"' selected>"+v.name+"</option>";
                                            }else{
                                                html+="<option value='"+v.id+"'>"+v.name+"</option>";
                                            }
