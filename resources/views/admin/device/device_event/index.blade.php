     ...
     <script>
            ...
            table.on('tool(device_event)', function(obj){
                ...
                if(obj.event === 'del'){
                    ...
                    layer.confirm('真的删除行么', function(index){
                        $.ajax({ 
                            type: "POST",
                            url: '{{url("api/admin/device/device_event")}}/'+ data.id,
                            data: {
                                '_method': 'DELETE',
                                'name': data.value,
                            },
                    ...
                    layer.open({
                        ...
                        shade:0.8,
                        area:['100%','100%'],
                        content:'{{url("admin/device/device_event/edit")}}',
                        end:function(){
                            table.reload('device_event');
                        }
                    });
            ...
            $('#add-device_event-event').click(function(){
                layer.open({
                    ...
                    shade:0.8,
                    area:['100%','100%'],
                    content:'{{url("admin/device/device_event/add")}}',
                    end:function(){
                        table.reload('device_event');
                    }
                });
            ...
            form.on('select(device_region_id)', function(data){
                ...
                $.ajax({ 
                    type: "GET",
                    url: '{{url("api/admin/device_room/all")}}?device_region_id='+data.value,
                    success: function(result){
            ...
            form.on('select(device_id)', function(data){
                ...
                $.ajax({ 
                    type: "GET",
                    url: '{{url("api/admin/device/device_field/all")}}?device_id='+data.value,
                    success: function(result){
                        layer.close(ajaxLoad5);
                        if (result.code > 0) {
                            layer.msg(result.msg);
