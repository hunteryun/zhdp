 ...
 <body>
    <div class="layui-container" style="padding:15px">
     ...
     <script>
        ...
        //  获取父页面传过来的值
        var pest_warning_info =  window.parent.edit_pest_warning_info;
        //  初始化input
        $("input[name='title']").val(pest_warning_info.title);
        $("select[name='type']").val(pest_warning_info.type);
        laydate.render({
            elem: '#start_time' 
            ,type: 'datetime'
            ,calendar: true
            ,value: pest_warning_info.start_time
        });
        laydate.render({
                elem: '#end_time' 
                ,type: 'datetime'
                ,calendar: true
                ,value: pest_warning_info.end_time
            });
        $("textarea[name='warning']").val(pest_warning_info.warning);
        $("textarea[name='content']").val(pest_warning_info.content);
         //监听提交
         form.on('submit(formSubmit)', function(data) {
             ...
             $.ajax({ 
                type: "POST",
                url: '{{url("api/admin/pest_warning")}}/' + pest_warning_info.id,
                data: {
                    '_method': 'PUT',
