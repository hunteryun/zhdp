 ...
 <body>
    <div class="layui-container" style="padding:15px">
     ...
     <script>
        //  获取父页面传过来的值
        var product_field_info =  window.parent.edit_product_field_info;
        $('input[name="name"]').val(product_field_info.name);
        $('input[name="field"]').val(product_field_info.field);
        $('input[name="field_type_id"]').val(product_field_info.field_type_id);
        $('input[name="field_type_length"]').val(product_field_info.field_type_length);
        $('input[name="default"]').val(product_field_info.default);
        $('input[name="common_field"]').val(product_field_info.common_field);
        $('input[name="common_field_sort"]').val(product_field_info.common_field_sort);
        $('textarea[name="desc"]').val(product_field_info.desc);
        $('input[name="sort"]').val(product_field_info.sort);
        //  获取区域列表
        ...
        $.ajax({ 
            type: "GET",
            url: '{{url("api/user/field_type/all")}}',
            success: function(result){
                ...
                if (result.code > 0) {
                    ...
                    $.each(result.data,function(key,value){
                        if(value.id == product_field_info.field_type_id){
                            html+="<option value='"+value.id+"' selected>"+value.name+"|max-length:"+value.field_type_length+"</option>";
                        }else{
        ...
        form.on('select(field_type_id)', function(data){
         ...
         form.on('submit(formSubmit)', function(data) {
             ...
             $.ajax({ 
                type: "POST",
                url: '{{url("api/user/product/product_field")}}/' + product_field_info.id,
                data: {
                    '_method': 'PUT',
