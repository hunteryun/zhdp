 ...
 <body>
    <div class="layui-container" style="padding:15px">
     ...
     <script>
        ...
        //  获取父页面传过来的值
        var login_notice_log_info =  window.parent.login_notice_log_info;
        // 获取通知详情
        ...
        $.ajax({ 
            type: "GET",
            url: '{{url("api/user/login_notice/login_notice_log")}}/' + login_notice_log_info.id,
            success: function(result){
                layer.close(ajaxLoad);
                if (result.code > 0) {
                    layer.msg(result.msg);
                } else {
                    login_notice = result.data.login_notice;
                    $("input[name='title']").val(login_notice.title);
                    $("textarea[name='content']").val(login_notice.content);
                    $("select[name='type']").val(login_notice.type);
                    form.render("select");
                    // 初始化编辑器
