...
class ArticleCommentController extends Base
{
    ...
    public function update(Request $request, $id){
        ...
        $articleCommentInfo = AdminModel::where('token', $this->admin_token())->firstOrFail()->article_comment()->where('id', $id)->firstOrFail();
    ...
    public function destroy(Request $request, $id){
        ...
        $deleteDeviceRegionStatus = AdminModel::where('token', $this->admin_token())->firstOrFail()->article_comment()->where('id', $id)->firstOrFail()->delete();
    ...
    {
        $limit      = $request->input('limit');
        $deviceRegionList = AdminModel::where('token', $this->admin_token())->firstOrFail()->article_comment()->with('user', 'article')->orderBy('id', 'desc')->paginate($limit)->toArray();
        $returnData = [];
        $returnData['msg']              = "查询成功";
        $returnData['count']            = $deviceRegionList['total'];
        $returnData['current_page']     = $deviceRegionList['current_page'];
