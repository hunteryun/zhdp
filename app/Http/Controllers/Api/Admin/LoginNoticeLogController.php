...
class LoginNoticeLogController extends Base
{
    ...
    public function index(Request $request)
    {
        ...
        $loginNoticeLogList = AdminModel::where('token', $this->admin_token())->firstOrFail()->login_notice_log()->orderBy("id", "desc")->with('login_notice')->paginate($limit)->toArray();
        $returnData = [];
        $returnData['msg']              = "查询成功";
    ...
    {
        $loginNoticeLogAll = AdminModel::where('token', $this->admin_token())->firstOrFail()->login_notice_log()->where('status', '0')->orderBy("id", "desc")->with('login_notice')->get();
        // 获取后变为已读
    ...
    {
        $loginNoticeLogInfo = AdminModel::where('token', $this->admin_token())->firstOrFail()->login_notice_log()->where('id', $id)->with('login_notice')->firstOrFail();
        // 更新查看状态
