...
class ArticleCollectionController extends Base
{
    ...
    public function index(Request $request)
    {
        ...
        $articleCollectionList = AdminModel::where('token', $this->admin_token())->firstOrFail()->article_collection()->paginate($limit)->toArray();
        $returnData = [];
        $returnData['msg']              = "查询成功";
    ...
    {
        $articleExists = AdminModel::where('token', $this->admin_token())->firstOrFail()->article_collection()->where('article_id', $article_id)->exists();
        if($articleExists){
     ...
     public function store(Request $request)
    {
        ...
        $articleExists = AdminModel::where('token', $this->admin_token())->firstOrFail()->article_collection()->where('article_id', $article_id)->exists();
        if($articleExists){
            $deleteArticleCollectionStatus = AdminModel::where('token', $this->admin_token())->firstOrFail()->article_collection()->where('article_id', $article_id)->firstOrFail()->delete();
            ...
            $user_id = AdminModel::where('token', $this->admin_token())->firstOrFail(['id'])->id;
    ...
    public function destroy(Request $request, $id){
        $deleteArticleCollectionStatus = AdminModel::where('token', $this->admin_token())->firstOrFail()->article_collection()->where('id', $id)->firstOrFail()->delete();
        if(!$deleteArticleCollectionStatus){
    ...
    {
        $limit  = $request->input('limit');
        $deviceRegionList = AdminModel::where('token', $this->admin_token())->firstOrFail()->article_collection()->whereHas('article', function($query){
            $request = request();
