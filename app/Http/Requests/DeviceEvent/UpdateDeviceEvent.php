...
namespace App\Http\Requests\DeviceEvent;
use App\Http\Requests\Base;
use App\Rules\DeviceIdExists; // 引入判断设备id是否存在
use App\Rules\DeviceFieldIdExists; // 引入判断设备字段id是否存在
use App\Rules\DeviceRegionIdExists; // 引入判断设备区域id是否存在
use App\Rules\DeviceRoomIdExists; // 引入判断设备房间id是否存在
// 设备事件
class UpdateDeviceEvent extends Base
{
    public $messages = [
        'name.required' => '设备事件名必填!',
        'name.alpha_dash' => '名字只允许字母和数字，以及破折号和下划线!',
        'name.between' => '名字长度需要在1-30之间!',
        'type.required' => '设备事件类型名必填!',
        'type.numeric' => '设备事件类型必须是数字!',
        'type.in' => '设备事件类型必须in 0,1,2!',
        'value.required' => '设备事件值必填!',
        'value.alpha_dash' => '值只允许字母和数字，以及破折号和下划线!',
        'value.between' => '设备事件值长度需要在1-255之间!',
        // 'desc.required' => '设备事件描述必填!',
        // 'desc.alpha_dash' => '描述只允许字母和数字，以及破折号和下划线!',
        'desc.max' => '设备事件描述最大长度255!',

        'device_region_id.required' => '设备区域id必填!',
        'device_region_id.numeric' => '设备区域id必须是数字!',
    ...
    {
        //false 表示设备事件无权限，如果要带入控制器设置为true
        return false;
    }
